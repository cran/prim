...
<<fig=FALSE,print=FALSE,results=hide>>=
xlim <- c(-30, 550)
ylim <- c(-30, 550)
...
HDR directly: 
<<>>=
qflow.hdr.plus <- prim.box(x=xflow, y=yflow, threshold=0.85, threshold.type=1)
@ 
...
qflow.prim2.disp <- qflow.prim2
for (i in 1:qflow.prim2$num.hdr.class)
  qflow.prim2.disp$x[[i]] <- qflow.prim2.disp$x[[i]][1:30,]
plot(qflow.prim2.disp, col=col, xmin=xmin, xmax=xmax)
@ 
\end{center}
